...
// Cloudflare Git 연동 재배포 트리거용 확인 커밋입니다.
export default {
...
<url><loc>https://mingka.tcflick.com/affiliate.html</loc><lastmod>2026-09-13</lastmod><changefreq>monthly</changefreq><priority>0.5</priority></url>
    ...
    html = html.replace(/gtag\(['"]config['"],\s*['"]G-06DTYM04S2['"]\);/i, `if (localStorage.getItem("mingka_internal") !== "1") { gtag("config", "G-06DTYM04S2"); }`);
    ...
    if (/<section\s+id=["']quiz["'][\s\S]*?<\/section>/i.test(html)) html = html.replace(/(<section\s+id=["']quiz["'][\s\S]*?<\/section>)/i, `$1\n${middleAd}`);
